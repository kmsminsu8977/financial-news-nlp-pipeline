...
이 문서는 `financial-news-nlp-pipeline` 컨텐츠를 만들 때 참고해야 할 로컬 source inventory입니다. 새 코드, 데이터 예시, 문서, 산출물을 만들 때는 이 파일과 `AGENTS.md`를 먼저 확인합니다.

## 프로젝트 식별

- 프로젝트명: 금융 뉴스 NLP 파이프라인 (Financial News NLP Pipeline)
- 연구 카테고리: Financial Data Science
- 핵심 질문: 뉴스 텍스트를 매매나 리스크 분석에 쓰려면 어떤 전처리 산출물이 먼저 필요한가?

## KAIST-DFMBA 참고 경로

아래 경로는 컨텐츠를 설계할 때 확인할 로컬 참고 위치입니다. 세부 구현은 이 저장소의 목적에 맞게 별도 구조로 정리합니다.

- KAIST-DFMBA/Financial-Big-Data-Analysis/
- KAIST-DFMBA/LLM-and-RAG-Language-Model/
- KAIST-DFMBA/DFMBA-Assistant/

## 참고 초점

- 텍스트 수집, 정제, 토큰화 흐름
- 감성 lexicon 또는 label을 산출물로 남기는 방식
- 기사 단위 feature table 구성
