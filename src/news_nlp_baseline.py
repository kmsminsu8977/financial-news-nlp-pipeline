...
PROJECT = {"analysis_type": "news_nlp", "category": "Financial Data Science", "columns": ["article_id", "published_at", "symbol", "headline", "summary"], "methods": ["headline과 summary를 소문자 토큰으로 정리하고 불용기호를 제거한다.", "작은 금융 감성 lexicon으로 긍정/부정 단어 수와 label을 계산한다.", "기사 문장은 합성 문장으로 작성해 원 강의자료를 사용하지 않았다."], "module_name": "news_nlp_baseline", "objective": "금융 뉴스 텍스트를 정제하고 간단한 감성 점수와 토큰 통계를 산출한다.", "question": "뉴스 원문을 매매나 리스크 분석에 쓰려면 어떤 전처리 산출물이 먼저 필요한가?", "rows": [["N001", "2026-03-01T08:10:00", "AAA", "AAA raises guidance after resilient demand", "management cited stronger backlog and stable margins"], ["N002", "2026-03-01T09:25:00", "BBB", "BBB faces margin pressure from higher funding cost", "analysts warned that credit spreads may slow expansion"], ["N003", "2026-03-02T07:50:00", "CCC", "CCC announces buyback and improved cash flow", "the board approved capital return after debt reduction"], ["N004", "2026-03-02T10:40:00", "AAA", "AAA supplier disruption raises delivery risk", "temporary shutdown may delay high margin products"]], "sample_name": "news_articles.csv", "title_en": "Financial News NLP Pipeline", "title_ko": "금융 뉴스 NLP 파이프라인"}
